...
local existing = TargetParent:FindFirstChild("HexaX")
if existing then
	existing:Destroy()
end
...
local Theme = {
	...
	Active = Color3.fromRGB(232, 232, 232),
	ActiveText = Color3.fromRGB(0, 0, 0),
	ToggleOn = Color3.fromRGB(255, 211, 46),
...
local function makeDraggable(frame: GuiObject, handle: GuiObject?)
	...
	UserInputService.InputChanged:Connect(function(input)
		if dragging and dragInput and input == dragInput and dragStart and startPos then
			...
			tween(frame, TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
local function addHover(button: TextButton, idleBg: Color3, hoverBg: Color3, activeBg: Color3?)
	...
	button.MouseEnter:Connect(function()
		if button:GetAttribute("HexaVipOnly") == true and not HEXA_IS_VIP then
			tween(button, ti, {BackgroundColor3 = Color3.fromRGB(52, 47, 24)})
			return
		end
		local isToggle = button:GetAttribute("IsToggle")
		local isActive = button:GetAttribute("IsActive") == true
		if isToggle then
			tween(button, ti, {BackgroundColor3 = hoverBg})
		else
	...
	button.MouseLeave:Connect(function()
		if button:GetAttribute("HexaVipOnly") == true and not HEXA_IS_VIP then
			tween(button, ti, {BackgroundColor3 = Color3.fromRGB(70, 65, 45)})
			return
		end
		local isToggle = button:GetAttribute("IsToggle")
		local isActive = button:GetAttribute("IsActive") == true
		if isToggle then
			tween(button, ti, {BackgroundColor3 = idleBg})
		else
			tween(button, ti, {BackgroundColor3 = isActive and (activeBg or Theme.Active) or idleBg})
		end
	end)
end
...
local function createSlider(parent: Instance, title: string, minVal: number, maxVal: number, defaultVal: number, posY: number, onChanged: (number) -> (), vipOnly: boolean?)
	...
	local segmentCount = MOBILE_DEVICE and 14 or 20
	local segments = {}
	...
	local function effectiveMax()
		...
		return math.max(minVal, math.min(originalMax, 400))
	end
	...
	function controller.Refresh()
		...
		local position = math.clamp((currentValue - minVal) / math.max(1, allowedMax - minVal), 0, 1)
	...
	local function setFromInput(input: InputObject)
		...
		currentValue = math.floor(minVal + (allowedMax - minVal) * position + 0.5)
		controller.Refresh()
	end
...
notifyVipLocked = function()
	...
	VipNotificationText.Text = Lang.Current == "EN"
		and "Join the Discord to learn how to obtain VIP."
		or "Únete al Discord para saber cómo obtener VIP."
...
Escribe el nombre de una función para encontrarla. Los usuarios VIP pueden marcar funciones con la estrella y acceder a ellas desde FAVORITOS.
...
Type a feature name to find it. VIP users can mark features with the star and access them from FAVORITES.
...
Options displaying the VIP label require an active VIP key.]]},
	...
	{"PUNTERÍA AUTOMÁTICA (CABEZA)", "AUTO AIM (HEAD)"},
	{"PUNTERÍA AUTOMÁTICA (CUERPO)", "AUTO AIM (BODY)"},
	...
	{"CAÍDA", "DROP"},
	{"EXPLOSIÓN", "BURST"},
	{"HEXA X - SISTEMA DE CLAVE", "HEXA X - KEY SYSTEM"},
	...
	{"¡COPIADO!", "COPIED!"},
	{"Por Nony", "By Nony"},
	{"HEXA X  •  POR NONY", "HEXA X  •  BY NONY"},
	{"HEXA X - TUTORIAL", "HEXA X - TUTORIAL"},
	{"ENTENDIDO", "GOT IT"},
	{"¿CERRAR HEXA X?", "CLOSE HEXA X?"},
	{"CANCELAR", "CANCEL"},
	{"SÍ", "YES"},
...
mkStroke(KeyFrame, Theme.Accent, 0.4, 2)
...
KeyTitle.Text = "HEXA X - SISTEMA DE CLAVE"
...
mkStroke(KeyBoxContainer, Theme.Accent, 0.5, 1)
...
mkStroke(MainFrame, Theme.Accent, 0.56, 2)
...
SnowBackground.Name = "HexaPanelSnow"
...
SnowBackground.ClipsDescendants = true
SnowBackground.Active = false
SnowBackground.ZIndex = 1
SnowBackground.Parent = MainFrame

local snowRandom = Random.new()
local snowSymbols = {"❄", "❅", "❆"}
local snowCount = MOBILE_DEVICE and 16 or 26

local function createPanelSnowflake(index: number)
	local flake = Instance.new("TextLabel")
	flake.Name = "HexaSnowflake" .. index
	flake.BackgroundTransparency = 1
	flake.Size = UDim2.fromOffset(28, 28)
	flake.AnchorPoint = Vector2.new(0.5, 0.5)
	flake.Text = snowSymbols[(index - 1) % #snowSymbols + 1]
	flake.TextColor3 = Theme.Accent
	flake.TextSize = snowRandom:NextInteger(8, MOBILE_DEVICE and 15 or 18)
	flake.Font = Enum.Font.GothamBold
	flake.TextTransparency = snowRandom:NextNumber(0.52, 0.82)
	flake.Active = false
	flake.ZIndex = 1
	flake.Parent = SnowBackground

	task.spawn(function()
		task.wait(snowRandom:NextNumber(0, 5))
		while flake.Parent and MainFrame.Parent do
			while flake.Parent and not MainFrame.Visible do
				task.wait(0.4)
			end
			if not flake.Parent then break end
			local startX = snowRandom:NextNumber(0.02, 0.98)
			local endX = math.clamp(startX + snowRandom:NextNumber(-0.12, 0.12), 0.02, 0.98)
			flake.Position = UDim2.new(startX, 0, 0, -24)
			flake.Rotation = snowRandom:NextInteger(-35, 35)
			flake.TextTransparency = snowRandom:NextNumber(0.52, 0.82)
			local fall = tween(flake, TweenInfo.new(
				snowRandom:NextNumber(5.5, 11),
				Enum.EasingStyle.Linear,
				Enum.EasingDirection.InOut
			), {
				Position = UDim2.new(endX, 0, 1, 28),
				Rotation = flake.Rotation + snowRandom:NextInteger(90, 260),
			})
			fall.Completed:Wait()
			task.wait(snowRandom:NextNumber(0.1, 1.4))
		end
	end)
end

for index = 1, snowCount do
	createPanelSnowflake(index)
end
...
Header.Size = UDim2.new(1, 0, 0, 106)
Header.ZIndex = 4
...
Title.Size = UDim2.new(1, -210, 0, 58)
Title.Text = "Hexa X"
...
if HEXA_IS_OWNER then
	Title.Size = UDim2.new(1, -274, 0, 58)
...
Content.Position = UDim2.new(0, 0, 0, 108)
Content.Size = UDim2.new(1, 0, 1, -108)
...
registerFunctionButton = function(button)
	if not button:IsA("TextButton") or button:GetAttribute("HexaNoFavorite") == true then return end
	if not button:IsDescendantOf(Content) or button:IsDescendantOf(FavoritesCard) then return end
	if button:FindFirstAncestor("HexaPlayerDropdown") then return end
	local parentScrolling = button:FindFirstAncestorOfClass("ScrollingFrame")
	if parentScrolling and parentScrolling ~= Content then return end
	local id = getFavoriteId(button)
	if id == "" or FavoriteRegistry[id] then return end
	local card = getCardForButton(button)
	FavoriteRegistry[id] = {Button = button, Card = card}
	local star = Instance.new("TextButton")
	star.Name = "HexaFavoriteStar"
	star.AnchorPoint = Vector2.new(1, 0.5)
	local starOffset = -8
	if button:GetAttribute("HexaVipOnly") == true then
		starOffset = -116
	elseif button:GetAttribute("IsToggle") == true then
		starOffset = -76
	end
	star.Position = UDim2.new(1, starOffset, 0.5, 0)
	...
	star.ZIndex = math.max(button.ZIndex + 8, 12)
	star.Visible = HEXA_IS_VIP
	star.Parent = button
	star:SetAttribute("HexaNoTranslate", true)
	FavoriteStars[id] = star
	star.MouseButton1Click:Connect(function()
		if not HEXA_IS_VIP then requireVip(); return end
		FavoriteIds[id] = not FavoriteIds[id]
		if not FavoriteIds[id] then FavoriteIds[id] = nil end
		saveFavorites()
		refreshFavoritesCard()
	end)
	task.defer(refreshFavoritesCard)
end
...
applySearchFilter = function()
	...
	for _, card in ipairs(Content:GetChildren()) do
		if card:IsA("Frame") and card:GetAttribute("HexaSearchableCard") == true then
			...
			local visible = query == "" and defaultVisible or (query ~= "" and string.find(cardSearchText(card), query, 1, true) ~= nil)
			card.Visible = visible
			if visible and query ~= "" then matches += 1 end
			for _, object in ipairs(card:GetDescendants()) do
	...
	NoResultsLabel.Visible = query ~= "" and matches == 0
end

SearchBox:GetPropertyChangedSignal("Text"):Connect(applySearchFilter)
Content.DescendantAdded:Connect(function()
	task.defer(applySearchFilter)
end)
...
local CombatCard = sectionCard(MOBILE_DEVICE and 440 or 348)
...
local headAimKey: any = MOBILE_DEVICE and "AUTO" or nil
local bodyAimKey: any = MOBILE_DEVICE and "AUTO" or nil
...
local headKeybindBtn = createKeybindButton(
	...
	function(key) headAimKey = key end
)
local autoAimBodyButton = createToggleButton(
	CombatCard,
	"PUNTERÍA AUTOMÁTICA (CUERPO)",
	MOBILE_DEVICE and UDim2.new(1, -32, 0, 38) or UDim2.new(0.62, 0, 0, 38),
...
local bodyKeybindBtn = createKeybindButton(
	...
	function(key) bodyAimKey = key end
)
local ignoreFriendsButton = createToggleButton(CombatCard, "IGNORAR AMIGOS", UDim2.new(1, -32, 0, 38), MOBILE_DEVICE and UDim2.new(0, 16, 0, 280) or UDim2.new(0, 16, 0, 188))
local fovButton = createToggleButton(CombatCard, "USAR CÍRCULO FOV", UDim2.new(1, -32, 0, 38), MOBILE_DEVICE and UDim2.new(0, 16, 0, 326) or UDim2.new(0, 16, 0, 234))
local fovSlider = createSlider(CombatCard, "Radio del FOV", 30, 800, fovRadius, MOBILE_DEVICE and 372 or 280, function(v)
	fovRadius = v
	FovCircle.Size = UDim2.new(0, fovRadius * 2, 0, fovRadius * 2)
end)
...
AimHighlight.OutlineColor = Color3.fromRGB(0, 0, 0)
AimHighlight.OutlineTransparency = 0.1
...
ConfirmTitle.Text = "¿CERRAR HEXA X?"
...
local function getTutorialTargetSize()
	local viewport = workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize or GUI_VIEWPORT_SIZE
	return UDim2.fromOffset(
...
TutorialFrame.Name = "HexaTutorialFrame"
...
TutorialFrame.BackgroundColor3 = Theme.BG
TutorialFrame.BackgroundTransparency = 0.16
TutorialFrame.BorderSizePixel = 0
TutorialFrame.ClipsDescendants = true
TutorialFrame.ZIndex = 60
TutorialFrame.Parent = ScreenGui
mkCorner(TutorialFrame, 16)
mkStroke(TutorialFrame, Theme.Accent, 0.52, 2)

local TutorialHeader = Instance.new("Frame")
TutorialHeader.BackgroundColor3 = Theme.Panel
TutorialHeader.BackgroundTransparency = 0.24
TutorialHeader.BorderSizePixel = 0
TutorialHeader.Size = UDim2.new(1, 0, 0, 52)
TutorialHeader.ZIndex = 61
TutorialHeader.Parent = TutorialFrame

local TutTitle = Instance.new("TextLabel")
TutTitle.BackgroundTransparency = 1
TutTitle.Size = UDim2.new(1, -24, 1, 0)
TutTitle.Position = UDim2.new(0, 12, 0, 0)
TutTitle.Text = "HEXA X - TUTORIAL"
TutTitle.TextColor3 = Theme.TextMain
TutTitle.TextSize = 16
TutTitle.Font = Enum.Font.GothamBold
TutTitle.TextXAlignment = Enum.TextXAlignment.Left
TutTitle.ZIndex = 62
TutTitle.Parent = TutorialHeader
makeDraggable(TutorialFrame, TutTitle)

local TutorialScroll = Instance.new("ScrollingFrame")
TutorialScroll.BackgroundColor3 = Theme.Panel
TutorialScroll.BackgroundTransparency = 0.30
TutorialScroll.BorderSizePixel = 0
TutorialScroll.Position = UDim2.new(0, 14, 0, 64)
TutorialScroll.Size = UDim2.new(1, -28, 1, -126)
TutorialScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
TutorialScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
TutorialScroll.ScrollBarThickness = MOBILE_DEVICE and 6 or 4
TutorialScroll.ScrollBarImageColor3 = Theme.Accent
TutorialScroll.ScrollBarImageTransparency = 0.30
TutorialScroll.ZIndex = 61
TutorialScroll.Parent = TutorialFrame
mkCorner(TutorialScroll, 12)
mkStroke(TutorialScroll, Theme.Accent, 0.78, 1)

local TutText = Instance.new("TextLabel")
TutText.BackgroundTransparency = 1
TutText.Position = UDim2.new(0, 12, 0, 12)
TutText.Size = UDim2.new(1, -24, 0, 0)
TutText.AutomaticSize = Enum.AutomaticSize.Y
TutText.Text = [[PUNTERÍA AUTOMÁTICA
Activa la puntería a la cabeza o al cuerpo. En computadora puedes asignar una tecla o botón del ratón. En celular puedes elegir activación automática, al apuntar o al disparar.
...
TutText.TextColor3 = Theme.TextMain
TutText.TextSize = MOBILE_DEVICE and 12 or 13
TutText.TextWrapped = true
TutText.TextXAlignment = Enum.TextXAlignment.Left
TutText.TextYAlignment = Enum.TextYAlignment.Top
TutText.Font = Enum.Font.GothamMedium
TutText.ZIndex = 62
TutText.Parent = TutorialScroll

local CloseTutBtn = neonButton(TutorialFrame, "ENTENDIDO", UDim2.new(0, 140, 0, 38), UDim2.new(0.5, -70, 1, -50), 62)
CloseTutBtn.BackgroundColor3 = Theme.Panel2
CloseTutBtn.TextColor3 = Theme.TextOff
CloseTutBtn:SetAttribute("HexaNoFavorite", true)
...
local function updateResponsiveLayout()
	...
	if TutorialFrame.Visible and TutorialFrame.Size.X.Offset > 0 then TutorialFrame.Size = getTutorialTargetSize() end
	if OwnerFrame then
...
local flyConn, loopConn, renderConn, espConn, charAddedConn
local flyBV, flyBG
local noclipCache = {}
local speedBase = DEFAULT_WALK_SPEED
local jumpBase = DEFAULT_JUMP_POWER
...
local mobileAimTouchActive = false
local mobileAimTouchInput = nil
local mobileShotUntil = 0
local MobileToolConnections = setmetatable({}, {__mode = "k"})

local function bindMobileShotTool(tool)
	if not MOBILE_DEVICE or not tool:IsA("Tool") or MobileToolConnections[tool] then return end
	MobileToolConnections[tool] = tool.Activated:Connect(function()
		mobileShotUntil = os.clock() + 0.24
	end)
end
...
local function scanMobileTools(container)
	...
	container.ChildAdded:Connect(bindMobileShotTool)
end
...
if MOBILE_DEVICE then
	UserInputService.InputBegan:Connect(function(input, processed)
		...
		if camera and input.Position.X >= camera.ViewportSize.X * 0.45 then
			mobileAimTouchInput = input
			mobileAimTouchActive = true
		end
	end)
	UserInputService.InputEnded:Connect(function(input)
		if input == mobileAimTouchInput then
	...
	LocalPlayer.CharacterAdded:Connect(function(character) scanMobileTools(character) end)
end

local function isKeyActive(targetKey: any): boolean
	if MOBILE_DEVICE then
		local mode = type(targetKey) == "string" and targetKey or "AUTO"
		if mode == "AIM" then return mobileAimTouchActive end
		if mode == "FIRE" then return os.clock() <= mobileShotUntil end
		return true
	end
		...
		if player ~= LocalPlayer and player.Character then
			...
			if hum and hum.Health > 0 and targetRoot then
				...
				if dist3D <= maxAimDistance then
					if fovActive then
						local targetPart = (autoAimHeadActive and player.Character:FindFirstChild("Head")) or targetRoot
...
local function restoreNoclip()
	for part, original in pairs(noclipCache) do
...
local function cleanupMovement()
	...
	restoreNoclip()
	local _, hum = getCharacterData()
	if hum then pcall(function() hum.WalkSpeed = speedBase; hum.UseJumpPower = true; hum.JumpPower = jumpBase; hum.PlatformStand = false end) end
...
local function ensureFly()
	...
	flyBG = Instance.new("BodyGyro", root); flyBG.P = 9e4; flyBG.MaxTorque = Vector3.new(9e4, 9e4, 9e4); flyBG.CFrame = workspace.CurrentCamera.CFrame
	flyBV = Instance.new("BodyVelocity", root); flyBV.MaxForce = Vector3.new(9e4, 9e4, 9e4); flyBV.Velocity = Vector3.new(0,0,0)
	...
	flyConn = RunService.RenderStepped:Connect(function()
		...
		pcall(function() flyBG.CFrame = cam.CFrame; flyBV.Velocity = move * currentFlySpeed end)
	end)
end
...
speedButton.MouseButton1Click:Connect(function() speedActive = not speedActive; setActive(speedButton, speedActive); if speedActive then local _,h = getCharacterData() if h then speedBase=h.WalkSpeed end else local _,h=getCharacterData() if h then pcall(function() h.WalkSpeed=speedBase end) end end end)
jumpButton.MouseButton1Click:Connect(function() jumpActive = not jumpActive; setActive(jumpButton, jumpActive); if jumpActive then local _,h = getCharacterData() if h then jumpBase=h.JumpPower pcall(function() h.UseJumpPower=true end) end else local _,h=getCharacterData() if h then pcall(function() h.UseJumpPower=true; h.JumpPower=jumpBase end) end end end)
...
noclipButton.MouseButton1Click:Connect(function() noclipActive = not noclipActive; setActive(noclipButton, noclipActive); if not noclipActive then restoreNoclip() end end)
...
GotoButton.MouseButton1Click:Connect(function() if selectedTargetPlayer and selectedTargetPlayer.Character then local targetRoot = selectedTargetPlayer.Character:FindFirstChild("HumanoidRootPart") local _, _, root = getCharacterData() if root and targetRoot then pcall(function() root.CFrame = targetRoot.CFrame + Vector3.new(0, 3, 0) end) end end end)
UserInputService.JumpRequest:Connect(function() if infiniteJumpActive then local _, hum = getCharacterData() if hum and hum.Health > 0 then pcall(function() hum:ChangeState(Enum.HumanoidStateType.Jumping) end) end end end)
...
CheckKeyBtn.MouseButton1Click:Connect(function()
	...
	local normalKey = entered == "h3xa-ny"
	...
	if noclipActive then
		for _, inst in ipairs(char:GetDescendants()) do
			if inst:IsA("BasePart") then
				if noclipCache[inst] == nil then noclipCache[inst] = inst.CanCollide end
				inst.CanCollide = false
			end
		end
	else
		if next(noclipCache) ~= nil then restoreNoclip() end
	end
end)
...
renderConn = RunService.RenderStepped:Connect(function()
	local isHeadActive = autoAimHeadActive and isKeyActive(headAimKey)
	local isBodyActive = autoAimBodyActive and isKeyActive(bodyAimKey)

	if isHeadActive or isBodyActive then
		local targetPlayer = getClosestPlayer()
		if targetPlayer and targetPlayer.Character then
			local targetPart = (isHeadActive and targetPlayer.Character:FindFirstChild("Head")) or targetPlayer.Character:FindFirstChild("UpperTorso") or targetPlayer.Character:FindFirstChild("HumanoidRootPart")
			if targetPart then
				if AimHighlight.Adornee ~= targetPlayer.Character then AimHighlight.Adornee = targetPlayer.Character end
				local cam = workspace.CurrentCamera
				if cam then cam.CFrame = CFrame.new(cam.CFrame.Position, targetPart.Position) end
			else
...
TutorialBtn.MouseButton1Click:Connect(function()
	animateOpen(TutorialFrame, getTutorialTargetSize(), "BACK")
end)

CloseTutBtn.MouseButton1Click:Connect(function()
	local closeTutorial = tween(TutorialFrame, tweenOut, {Size = UDim2.new(0, 0, 0, 0)})
...
task.spawn(function()
	local success, errorMessage = pcall(function()
		...
		function X:setupCrosshair()
			...
			self.CrosshairCard.LayoutOrder = 31
		...
		function X:setupPlayerTools()
			...
			self:connect(self.InspectorButton.MouseButton1Click, function()
				self.InspectorEnabled = not self.InspectorEnabled
		...
		function X:updatePlayerInspector(now, force)
			...
			local target = selectedTargetPlayer
			if not target then
				self.InspectorText.Text = english and "SELECT A PLAYER" or "SELECCIONA UN JUGADOR"
				return
			end
		...
		function X:updateInfo(now)
			self.Frames += 1
			if now - self.LastFpsTime >= 0.5 then
		...
		function X:setupMisc()
			self.MiscCard, self.MiscTitle = self:makeCard(492, "MISCELÁNEO")
			self.MiscCard.LayoutOrder = 70
			self.AntiAfkButton = createToggleButton(self.MiscCard, "ANTI AUSENCIA (AFK)", UDim2.new(1, -32, 0, 38), UDim2.new(0, 16, 0, 44))
			self.ReconnectButton = neonButton(self.MiscCard, "RECONECTAR AL SERVIDOR", UDim2.new(1, -32, 0, 38), UDim2.new(0, 16, 0, 90))
			self.AutoRespawnButton = createToggleButton(self.MiscCard, "REAPARICIÓN AUTOMÁTICA", UDim2.new(1, -32, 0, 38), UDim2.new(0, 16, 0, 136))
			self.AutoEquipButton = createToggleButton(self.MiscCard, "EQUIPAR HERRAMIENTA AUTOMÁTICAMENTE", UDim2.new(1, -32, 0, 38), UDim2.new(0, 16, 0, 182))
			self.PerformanceButton = createToggleButton(self.MiscCard, "MODO RENDIMIENTO: CARTÓN", UDim2.new(1, -32, 0, 38), UDim2.new(0, 16, 0, 228))
			self.StreamerButton = createToggleButton(self.MiscCard, "MODO TRANSMISIÓN", UDim2.new(1, -32, 0, 38), UDim2.new(0, 16, 0, 274))
			self.AutoClickerButton = createToggleButton(self.MiscCard, MOBILE_DEVICE and "AUTO TOQUES" or "AUTOCLICKER", UDim2.new(1, -32, 0, 38), UDim2.new(0, 16, 0, 320))
			self.AutoClickCpsSlider = createSlider(self.MiscCard, MOBILE_DEVICE and "Toques por segundo" or "Clics por segundo", 1, 100, self.AutoClickCps, 366, function(value)
				self.AutoClickCps = value
			end, true)
			markVipControl(self.AutoClickerButton)
			self.PanicButton = neonButton(self.MiscCard, "ACTIVAR MODO PÁNICO", UDim2.new(1, -32, 0, 38), UDim2.new(0, 16, 0, 432))
			self.PanicButton.BackgroundColor3 = Theme.Danger
			self.PanicButton.TextColor3 = Theme.ActiveText
			...
			self:connect(self.AutoClickerButton.MouseButton1Click, function()
				if not requireVip() then return end
				self.AutoClicker = not self.AutoClicker
				self.LastAutoClick = 0
				self:toggle(self.AutoClickerButton, self.AutoClicker)
			end)
			self:connect(self.PanicButton.MouseButton1Click, function()
				self:panic()
				self.PanicButton.Text = "TODAS LAS FUNCIONES DESACTIVADAS"
			...
			self:connect(game.DescendantAdded, function(object)
				if not self.Performance then return end
				task.defer(function()
					if self.Performance and object then self:optimizePerformanceObject(object) end
				end)
			end)
			self:addReset(function()
				self.AntiAfk = false
				...
				self:toggle(self.AutoClickerButton, false)
				self:toggle(self.PerformanceButton, false)
				self:toggle(self.StreamerButton, false)
			end)
		end

		function X:getAutoClickPosition()
			local camera = workspace.CurrentCamera
			if MOBILE_DEVICE and camera then return camera.ViewportSize / 2 end
			return UserInputService:GetMouseLocation()
		end

		function X:performAutoClick()
			if not MOBILE_DEVICE and type(mouse1click) == "function" then
				pcall(mouse1click)
				return
			end
			local camera = workspace.CurrentCamera
			local clickPosition = self:getAutoClickPosition()
			pcall(function()
		...
		function X:updateAutoClicker(now)
			if not self.AutoClicker or not HEXA_IS_VIP then return end
			local cps = math.clamp(self.AutoClickCps, 1, 100)
			if now - self.LastAutoClick < (1 / cps) then return end
			local clickPosition = self:getAutoClickPosition()
			if self:isPointerOverInterface(clickPosition) then return end
			self.LastAutoClick = now
			self:performAutoClick()
		end

		function X:cachePerformanceProperty(object, propertyName)
			if object == nil then return false end
			local objectCache = self.PerformanceCache[object]
			if not objectCache then
				objectCache = {}
				self.PerformanceCache[object] = objectCache
			end
			if objectCache[propertyName] ~= nil then return true end
			local ok, value = pcall(function() return object[propertyName] end)
			if not ok then return false end
			objectCache[propertyName] = {value = value}
			return true
		end

		function X:setPerformanceProperty(object, propertyName, value)
			if not self:cachePerformanceProperty(object, propertyName) then return end
			pcall(function() object[propertyName] = value end)
		end

		function X:optimizePerformanceObject(object)
			if object == nil then return end
			if typeof(object) == "Instance" and object:IsDescendantOf(ScreenGui) then return end
			...
			if object:IsA("ParticleEmitter") or object:IsA("Trail") or object:IsA("Beam")
				...
				self:setPerformanceProperty(object, "Enabled", false)
			elseif object:IsA("PointLight") or object:IsA("SpotLight") or object:IsA("SurfaceLight") then
				self:setPerformanceProperty(object, "Enabled", false)
			elseif object:IsA("Decal") or object:IsA("Texture") then
				self:setPerformanceProperty(object, "Transparency", 1)
			elseif object:IsA("PostEffect") then
				self:setPerformanceProperty(object, "Enabled", false)
			elseif object:IsA("Atmosphere") then
				self:setPerformanceProperty(object, "Density", 0)
				self:setPerformanceProperty(object, "Haze", 0)
				self:setPerformanceProperty(object, "Glare", 0)
			elseif object:IsA("Clouds") then
				self:setPerformanceProperty(object, "Cover", 0)
				self:setPerformanceProperty(object, "Density", 0)
			elseif object:IsA("Sky") or object:IsA("SurfaceAppearance") then
				self:setPerformanceProperty(object, "Parent", nil)
			elseif object:IsA("SpecialMesh") then
				self:setPerformanceProperty(object, "TextureId", "")
			elseif object:IsA("MeshPart") then
				self:setPerformanceProperty(object, "TextureID", "")
				self:setPerformanceProperty(object, "RenderFidelity", Enum.RenderFidelity.Performance)
				self:setPerformanceProperty(object, "CastShadow", false)
				self:setPerformanceProperty(object, "Reflectance", 0)
				self:setPerformanceProperty(object, "Material", Enum.Material.SmoothPlastic)
				self:setPerformanceProperty(object, "MaterialVariant", "")
			...
			elseif object:IsA("Model") then
				pcall(function()
					self:setPerformanceProperty(object, "LevelOfDetail", Enum.ModelLevelOfDetail.StreamingMesh)
				end)
			end
		end

		function X:applyPerformanceGlobals()
			self:setPerformanceProperty(Lighting, "GlobalShadows", false)
			self:setPerformanceProperty(Lighting, "Brightness", 1)
			self:setPerformanceProperty(Lighting, "EnvironmentDiffuseScale", 0)
			self:setPerformanceProperty(Lighting, "EnvironmentSpecularScale", 0)
			self:setPerformanceProperty(Lighting, "FogStart", 1000000)
			self:setPerformanceProperty(Lighting, "FogEnd", 1000000)

			local terrain = workspace:FindFirstChildOfClass("Terrain")
			if terrain then
				self:setPerformanceProperty(terrain, "WaterWaveSize", 0)
				self:setPerformanceProperty(terrain, "WaterWaveSpeed", 0)
				self:setPerformanceProperty(terrain, "WaterReflectance", 0)
				self:setPerformanceProperty(terrain, "WaterTransparency", 1)
				self:setPerformanceProperty(terrain, "Decoration", false)
			end

			pcall(function()
				local rendering = settings().Rendering
				self:setPerformanceProperty(rendering, "QualityLevel", Enum.QualityLevel.Level01)
				pcall(function()
					self:setPerformanceProperty(rendering, "MeshPartDetailLevel", Enum.MeshPartDetailLevel.Level01)
				end)
			end)
		end

		function X:restorePerformance()
			for object, properties in pairs(self.PerformanceCache) do
				local parentData = properties.Parent
				if parentData then pcall(function() object.Parent = parentData.value end) end
				for propertyName, data in pairs(properties) do
					if propertyName ~= "Parent" then
						pcall(function() object[propertyName] = data.value end)
					end
				end
			end
			table.clear(self.PerformanceCache)
		end

		function X:applyPerformance()
			local generation = self.PerformanceGeneration
			if not self.Performance then
				self:restorePerformance()
				return
			end

			self:applyPerformanceGlobals()
			task.spawn(function()
				local descendants = game:GetDescendants()
				for index, object in ipairs(descendants) do
					if not self.Performance or generation ~= self.PerformanceGeneration then return end
					self:optimizePerformanceObject(object)
					if index % 250 == 0 then RunService.Heartbeat:Wait() end
				end
			end)
		end
		...
		function X:applyStreamer()
			...
			if self.Watermark then self.Watermark.Text = self.Streamer and "HEXA X" or "HEXA X  •  POR NONY" end
		end
		...
		function X:setupPersonalization()
			...
			self.Watermark.Text = "HEXA X  •  POR NONY"
		...
		function X:isPointerOverInterface(position)
			...
			return contains(MainFrame) or contains(KeyFrame) or contains(ConfirmFrame) or contains(TutorialFrame)
		...
		function X:setupGlobalConnections()
			self:connect(RunService.RenderStepped, function()
				...
				self:updateInfo(now)
				self:updateAutoEquip(now)
				self:updateAutoClicker(now)
			end)
		...
		addVipStateListener(function(isVip)
			...
			pcall(function() X:toggle(X.AutoClickerButton, false) end)
	...
	if not success then
		warn("[Hexa X] Error al cargar la extensión modular: " .. tostring(errorMessage))
	end
end)
