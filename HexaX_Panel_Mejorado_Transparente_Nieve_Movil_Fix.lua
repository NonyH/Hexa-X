...
local function destroyExistingHexa(parent)
	...
	pcall(function()
		local existing = parent:FindFirstChild("HexaX")
...
local function createSlider(parent: Instance, title: string, minVal: number, maxVal: number, defaultVal: number, posY: number, onChanged: (number) -> (), vipOnly: boolean?)
	...
	local segmentCount = MOBILE_DEVICE and 8 or 16
	local segments = {}
...
if not parentedScreenGui and CoreGui then
	parentedScreenGui = pcall(function()
...
local snowStarted = false
local snowConnection = nil
local snowflakes = {}

local function startPanelSnow()
	if snowStarted or not SnowBackground.Parent then return end
	snowStarted = true

	local ok = pcall(function()
		local random = Random.new()
		local count = MOBILE_DEVICE and 6 or 14
		for index = 1, count do
			local flake = Instance.new("TextLabel")
			flake.Name = "HexaSnowflake" .. index
			flake.BackgroundTransparency = 1
			flake.AnchorPoint = Vector2.new(0.5, 0.5)
			flake.Size = UDim2.fromOffset(24, 24)
			flake.Text = "❄"
			flake.TextColor3 = Theme.Accent
			flake.TextSize = random:NextInteger(8, MOBILE_DEVICE and 13 or 17)
			flake.Font = Enum.Font.GothamBold
			flake.TextTransparency = random:NextNumber(0.58, 0.84)
			flake.Active = false
			flake.ZIndex = 1
			...
			snowflakes[index] = {
				object = flake,
				x = random:NextNumber(0.02, 0.98),
				y = random:NextNumber(-1, 1),
				speed = random:NextNumber(0.035, 0.075),
				drift = random:NextNumber(-0.025, 0.025),
				rotation = random:NextNumber(-30, 30),
				rotationSpeed = random:NextNumber(-22, 22),
			}
		end
		...
		snowConnection = RunService.RenderStepped:Connect(function(deltaTime)
			...
			for _, data in ipairs(snowflakes) do
				local flake = data.object
				if flake and flake.Parent then
					data.y += data.speed * deltaTime * 10
					data.x += data.drift * deltaTime
					data.rotation += data.rotationSpeed * deltaTime
					if data.y > 1.08 then
						data.y = -0.08
						data.x = random:NextNumber(0.02, 0.98)
					end
					if data.x < -0.04 then data.x = 1.04 elseif data.x > 1.04 then data.x = -0.04 end
					flake.Position = UDim2.new(data.x, 0, data.y, 0)
					flake.Rotation = data.rotation
				end
			end
		end)
	end)
