...
__BootstrapGui.Name = "HexaX_Bootstrap"
...
__BootstrapStroke.Transparency = 0.55
__BootstrapStroke.Thickness = 1
...
local MainFrameStroke = mkStroke(MainFrame, Theme.Accent, 0.56, 1)
...
local Snow = {started = false, connection = nil, flakes = {}}
Snow.Background = Instance.new("Frame")
Snow.Background.Name = "HexaPanelSnow"
Snow.Background.BackgroundTransparency = 1
Snow.Background.Size = UDim2.new(1, -4, 1, -4)
Snow.Background.Position = UDim2.new(0, 2, 0, 2)
Snow.Background.BorderSizePixel = 0
Snow.Background.ClipsDescendants = true
Snow.Background.Active = false
Snow.Background.ZIndex = 1
Snow.Background.Parent = MainFrame

function Snow.start()
	if Snow.started or not Snow.Background.Parent then return end
	Snow.started = true

	local ok = pcall(function()
		local random = Random.new()
		local count = MOBILE_DEVICE and 4 or 10
		...
		local snowAccumulator = 0
		Snow.connection = RunService.RenderStepped:Connect(function(deltaTime)
			snowAccumulator += deltaTime
			local snowStep = MOBILE_DEVICE and (1 / 30) or (1 / 45)
			if snowAccumulator < snowStep then return end
			deltaTime = snowAccumulator
			snowAccumulator = 0
			if not ScreenGui.Parent then
				if Snow.connection then Snow.connection:Disconnect() end
				return
			end
			if not MainFrame.Visible then return end

			for _, data in ipairs(Snow.flakes) do
...
Content.Position = UDim2.new(0, 2, 0, 108)
Content.Size = UDim2.new(1, -4, 1, -110)
...
mkStroke(Tutorial.Frame, Theme.Accent, 0.52, 2)
...
Tutorial.Title.Text = "HEXA X - TUTORIAL"
...
Tutorial.Scroll.ScrollBarImageColor3 = Theme.Accent
...
mkStroke(Tutorial.Scroll, Theme.Accent, 0.78, 1)
...
local function bindMobileShotTool(tool)
	...
	Runtime.MobileToolConnections[tool] = tool.Activated:Connect(function()
...
local function rebuildPlayerList()
	...
	for _, player in ipairs(Players:GetPlayers()) do
		if HexaSharedTargetFilters:AllowsPlayer(player, false) then
			...
			item.MouseButton1Click:Connect(function()
				if not HexaSharedTargetFilters:AllowsPlayer(player, false) then return end
...
HexaSharedTargetFilters:AddListener(function()
	rebuildPlayerList()
	if selectedTargetPlayer and not HexaSharedTargetFilters:AllowsPlayer(selectedTargetPlayer, false) then
...
task.spawn(function()
	local moduleOk, moduleError = xpcall(function()
		...
		local EspAdvancedCard = sectionCard(184)
		...
		local function applyFullbright()
			...
			if not ok then warn("[HEXA X / FULLBRIGHT] " .. tostring(applyError)) end
		end
			...
			for _, part in ipairs(character:GetDescendants()) do
				if part:IsA("BasePart") then
					for x = -1, 1, 2 do
						for y = -1, 1, 2 do
							for z = -1, 1, 2 do
								local offset = Vector3.new(part.Size.X * x * 0.5, part.Size.Y * y * 0.5, part.Size.Z * z * 0.5)
								local point = camera:WorldToViewportPoint(part.CFrame:PointToWorldSpace(offset))
								if point.Z > 0 then
									visiblePoints += 1
		...
		connect(RunService.RenderStepped, function(dt)
			if State.Dead then return end
			local now = os.clock()
			if Settings.NoRecoil or Settings.RapidFire or Settings.NoSpread or Settings.AutoReload or Settings.InfiniteAmmo then
				updateWeapons(now)
			else
				restoreWeapons()
			end
	...
	if not moduleOk then
		warn("[HEXA X / FUNCIONES ADICIONALES] " .. tostring(moduleError))
	end
end)
...
task.spawn(function()
	local essentialsOk, essentialsError = xpcall(function()
		...
		local function setInstantHit(enabled)
			...
			if State.InstantHitActive then scanProjectiles() end
		end
		...
		connect(workspace.DescendantAdded, function(object)
			if isProjectile(object) then
				State.Projectiles[object] = true
				if State.InstantHitActive then accelerateProjectile(object) end
			end
		end)
	...
	if not essentialsOk then
		warn("[HEXA X / FUNCIONES ESENCIALES] " .. tostring(essentialsError))
	end
end)
...
GotoButton.MouseButton1Click:Connect(function()
	...
	if not HexaSharedTargetFilters:AllowsPlayer(target, true) then return end
	local targetRoot = target.Character and target.Character:FindFirstChild("HumanoidRootPart")
	local _, _, root = getCharacterData()
	if root and targetRoot then pcall(function() root.CFrame = targetRoot.CFrame + Vector3.new(0, 3, 0) end) end
end)
	...
	if noclipActive then
		for _, inst in ipairs(char:GetDescendants()) do
			if inst:IsA("BasePart") then
				if Runtime.noclipCache[inst] == nil then Runtime.noclipCache[inst] = inst.CanCollide end
...
task.spawn(function()
	local success, errorMessage = pcall(function()
		...
		function X:setupMisc()
			...
			self.SystemCard, self.SystemTitle = self:makeCard(232, "SISTEMA Y UTILIDADES")
			...
			self.PerformanceButton = createToggleButton(self.SystemCard, "MODO RENDIMIENTO: CARTÓN", UDim2.new(1, -32, 0, 38), UDim2.new(0, 16, 0, 136))
			self.StreamerButton = createToggleButton(self.SystemCard, "MODO TRANSMISIÓN", UDim2.new(1, -32, 0, 38), UDim2.new(0, 16, 0, 182))
...
if __HexaOk then
	...
	warn("[Hexa X] Error de inicio: " .. tostring(__HexaError))
	pcall(function()
		...
		__BootstrapFrame.Size = UDim2.new(0, 320, 0, 180)
		__BootstrapTitle.Text = "ERROR AL ABRIR HEXA X"
